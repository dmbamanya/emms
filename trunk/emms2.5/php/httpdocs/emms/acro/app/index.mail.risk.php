...
$_html .= sprintf('<tr class="header"><td>&nbsp;</td><td class="header" colspan="2">%s%s</td></tr>','PAR 30 - ',WEBPAGE::$gt['SP.SCR.portfolioAtRisk']);

$_html .= sprintf('<tr class="header"><td class="header">%s</td><td class="header">%s</td><td class="header">%s</td><td class="header">%s</td><td class="header">&nbsp;Status</td></tr>',WEBPAGE::$gt['tblZones.zone'],$cdata[0]['lfdate'],$cdata[0]['fmrdate'],$cdata[0]['now_date']);
...
$_html .= sprintf('<tr><td class="totals">%s</td><td class="totals">%s%%</td><td class="totals">%s%%</td><td class="totals">%s%%</td><td>&nbsp;</td></tr>',WEBPAGE::$gt['total'],$total_cdata[0]['total_lfriskB'],$total_cdata[0]['total_fmr_riskB'],$total_cdata[0]['total_risk_B']);
...
if ($bAlertFlag) {
$subject = sprintf('Accion inmediata - PAR 30 Report: %s',WEBPAGE::$conf['app']['client_name']);}
else {$subject = sprintf('Normal - PAR 30 Report: %s',WEBPAGE::$conf['app']['client_name']);}
