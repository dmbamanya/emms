...
$dataC = array();
$dataD = array();
$riskA 		= WEBPAGE::$dbh->getAll(sprintf("select sum(lcd.balance_kp) as kp, l.zone_id, l.program_id, advisor_id from tblLoansCurrentData as lcd, tblLoans as l where l.id = lcd.loan_id and lcd.cn_delay >= '%s' group by l.zone_id, l.program_id, l.program_id, l.advisor_id", WEBPAGE::$conf['app']['risk.days.A']));
foreach($riskA as $key=>$val) { $dataA[$val['zone_id']][$val['program_id']][$val['advisor_id']] = $val['kp']; }
$riskB 		= WEBPAGE::$dbh->getAll(sprintf("select sum(lcd.balance_kp) as kp, l.zone_id, l.program_id, advisor_id from tblLoansCurrentData as lcd, tblLoans as l where l.id = lcd.loan_id and lcd.cn_delay >= '%s' group by l.zone_id, l.program_id, l.program_id, l.advisor_id", WEBPAGE::$conf['app']['risk.days.B']));
foreach($riskB as $key=>$val) { $dataB[$val['zone_id']][$val['program_id']][$val['advisor_id']] = $val['kp']; }
$riskC 		= WEBPAGE::$dbh->getAll(sprintf("select sum(lcd.balance_kp) as kp, l.zone_id, l.program_id, advisor_id from tblLoansCurrentData as lcd, tblLoans as l where l.id = lcd.loan_id and lcd.cn_delay >= '%s' group by l.zone_id, l.program_id, l.program_id, l.advisor_id", WEBPAGE::$conf['app']['risk.days.C']));
foreach($riskC as $key=>$val) { $dataC[$val['zone_id']][$val['program_id']][$val['advisor_id']] = $val['kp']; }
$riskD 		= WEBPAGE::$dbh->getAll(sprintf("select sum(lcd.balance_kp) as kp, l.zone_id, l.program_id, advisor_id from tblLoansCurrentData as lcd, tblLoans as l where l.id = lcd.loan_id and lcd.cn_delay >= '%s' group by l.zone_id, l.program_id, l.program_id, l.advisor_id", WEBPAGE::$conf['app']['risk.days.D']));
...
if (WEBPAGE::$conf['app']['auto_write_off']) {
  $lod_id = WEBPAGE::$dbh->getAll(sprintf("select max(lod.id) id,lcd.loan_id,lcd.balance_kp,lcd.balance_kaf,lcd.balance_kat,lcd.cn_date from tblLoansOnDelinquency lod, tblLoansCurrentData lcd where lcd.loan_id = lod.loan_id and lcd.cn_delay > '%s' group by lod.loan_id", WEBPAGE::$conf['app']['auto_write_off_margin']));
  foreach($lod_id as $key=>$val) {
    ...
    WEBPAGE::$dbh->query(sprintf("insert into tblLoanStatusHistory (id,loan_id,p_status,status,date,user_id,memo) values ('Null','%s','G','LI','%s','1','%s')",$val['loan_id'],$val['cn_date'],WEBPAGE::$gt['loanWriteOff']));
