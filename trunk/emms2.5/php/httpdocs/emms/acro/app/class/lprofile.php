...
class LPROFILE
{
	...
	$this->status_label = WEBPAGE::$gt[$tmp];
	...
	function createProfile($LoanID)
        {
          $sql = "SELECT
	  			...
	  			DATE_FORMAT(l.creator_date,'WEBPAGE::$gt['date_format_mysql']') AS creator_date,
	  			DATE_FORMAT(l.editor_date,'WEBPAGE::$gt['date_format_mysql']') AS editor_date,
	  			DATE_FORMAT(l.revised_date,'WEBPAGE::$gt['date_format_mysql']') AS revised_date,
	  			DATE_FORMAT(l.approval_date,'WEBPAGE::$gt['date_format_mysql']') AS approval_date,
	  			DATE_FORMAT(l.disburse_date,'WEBPAGE::$gt['date_format_mysql']') AS disburse_date,
	  			...
	  			DATE_FORMAT(l.delivered_date,'WEBPAGE::$gt['date_format_mysql']') AS delivered_date,
	  			UNIX_TIMESTAMP(l.delivered_date) AS delivered_date_utc,
	  			DATE_FORMAT(l.cancel_date,'WEBPAGE::$gt['date_format_mysql']') AS cancel_date,
	  			DATE_FORMAT(l.rejected_date,'WEBPAGE::$gt['date_format_mysql']') AS rejected_date,
	  			DATE_FORMAT(l.legal_in_date,'WEBPAGE::$gt['date_format_mysql']') AS legal_in_date,
	  			UNIX_TIMESTAMP(l.legal_in_date) AS legal_in_date_utc,
	  			DATE_FORMAT(l.legal_out_date,'WEBPAGE::$gt['date_format_mysql']') AS legal_out_date,
	  			DATE_FORMAT(l.retracted_date,'WEBPAGE::$gt['date_format_mysql']') AS retracted_date,
	  			DATE_FORMAT(l.r_open_date,'WEBPAGE::$gt['date_format_mysql']') AS r_open_date,
				t.borrower_type,
				t.calendar_type,
				t.payment_frequency,
				t.margin_k,
  ...
  if ($lprofile->revised_date) { $revised_date_field = sprintf("<tr><td class=recordviewlabel>%s</td><td class=recordviewvalue>%s</td><td class=recordviewvalue>%s</td></tr>",WEBPAGE::$gt['tblLoans.revised_id'],$lprofile->revised,$lprofile->revised_date); }	  		
  if ($lprofile->approval_date) { $approval_date_field = sprintf("<tr><td class=recordviewlabel>%s</td><td class=recordviewvalue>%s</td><td class=recordviewvalue>%s</td></tr>",WEBPAGE::$gt['tblLoans.approval_id'],$lprofile->approval,$lprofile->approval_date); }
  if ($lprofile->disburse_date) { $disburse_date_field = sprintf("<tr><td class=recordviewlabel>%s</td><td class=recordviewvalue>%s</td><td class=recordviewvalue>%s</td></tr>",WEBPAGE::$gt['tblLoans.disburse_id'],$lprofile->disburse,$lprofile->disburse_date); }
  if ($lprofile->delivered_date) { $delivered_date_field = sprintf("<tr><td class=recordviewlabel>%s</td><td class=recordviewvalue>%s</td><td class=recordviewvalue>%s</td></tr>",WEBPAGE::$gt['tblLoans.delivered_id'],$lprofile->delivered,$lprofile->delivered_date); }
  if ($lprofile->cancel_date) { $cancel_date_field = sprintf("<tr><td class=recordviewlabel>%s</td><td class=recordviewvalue>%s</td><td class=recordviewvalue>%s</td></tr>",WEBPAGE::$gt['tblLoans.cancel_id'],$lprofile->cancel,$lprofile->cancel_date); }
  if ($lprofile->rejected_date) { $rejected_date_field = sprintf("<tr><td class=recordviewlabel>%s</td><td class=recordviewvalue>%s</td><td class=recordviewvalue>%s</td></tr>",WEBPAGE::$gt['tblLoans.rejected_id'],$lprofile->rejected,$lprofile->rejected_date); }
  if ($lprofile->r_open_date) { $r_open_date_field = sprintf("<tr><td class=recordviewlabel>%s</td><td class=recordviewvalue>%s</td><td class=recordviewvalue>%s</td></tr>",WEBPAGE::$gt['tblLoans.r_open_id'],$lprofile->r_open,$lprofile->r_open_date); }
  if ($lprofile->legal_in_date) { $legal_in_date_field = sprintf("<tr><td class=recordviewlabel>%s</td><td class=recordviewvalue>%s</td><td class=recordviewvalue>%s</td></tr>",WEBPAGE::$gt['tblLoans.legal_in_id'],$lprofile->legal_in,$lprofile->legal_in_date); }
  if ($lprofile->legal_out_date) { $legal_out_date_field = sprintf("<tr><td class=recordviewlabel>%s</td><td class=recordviewvalue>%s</td><td class=recordviewvalue>%s</td></tr>",WEBPAGE::$gt['tblLoans.legal_out_id'],$lprofile->legal_out,$lprofile->legal_out_date); }

  if ( $lprofile->pg_value > 0 ) { $pg_value_field = sprintf("<tr><td class=recordviewvalue>%s</td><td class=recordviewvalue>%s</td><td align=right class=recordviewvalue>%s</td></tr>",WEBPAGE::$gt['tblCollateral.pawned_goods'],$lprofile->pg_memo,number_format($lprofile->pg_value,2,'.',',')); }
  if ( $lprofile->re_value > 0 ) { $re_value_field = sprintf("<tr><td class=recordviewvalue>%s</td><td class=recordviewvalue>%s</td><td align=right class=recordviewvalue>%s</td></tr>",WEBPAGE::$gt['tblCollateral.real_estate'],$lprofile->re_memo,number_format($lprofile->re_value,2,'.',',')); }
  if ( $lprofile->fgd_value > 0 ) { $fgd_value_field = sprintf("<tr><td class=recordviewvalue>%s</td><td class=recordviewvalue>%s</td><td align=right class=recordviewvalue>%s</td></tr>",WEBPAGE::$gt['tblCollateral.fiduciary_goods'],$lprofile->fgd_memo,number_format($lprofile->fgd_value,2,'.',',')); }
  if ( $lprofile->fgt_value > 0 ) { $fgt_value_field = sprintf("<tr><td class=recordviewvalue>%s</td><td class=recordviewvalue>%s</td><td align=right class=recordviewvalue>%s</td></tr>",WEBPAGE::$gt['tblCollateral.fiduciary_garantees'],$lprofile->fgt_memo,number_format($lprofile->fgt_value,2,'.',',')); }

  printf ("<h1 class=query>%s</h1>", WEBPAGE::$gt['MSG.TTL.029']);
  ?>
        ...
        <?
 		printf("
		  <table>
			...
			WEBPAGE::$gt['tblLoans.id'],$lprofile->loan_code,
			WEBPAGE::$gt['tblLoans.status'],$lprofile->status_label,
			WEBPAGE::$gt['tblClients.id'],coollink(sprintf("index.php?scr_name=%s",$lprofile->vscript),'id',$lprofile->client_id,WEBPAGE::$gt['details'],'recordview',$lprofile->client),
			WEBPAGE::$gt['tblLoans.kp'],number_format($lprofile->kp,2,'.',','),
			WEBPAGE::$gt['MSG.OTH.035'],$ltprofile->currency,
			WEBPAGE::$gt['tblBusiness.name'],coollink('index.php?scr_name=BS.SCR.browseBusiness','id',$bzprofile->id,WEBPAGE::$gt['details'],'recordview',$bzprofile->name),
			WEBPAGE::$gt['tblLoanTypes.description'],coollink('index.php?scr_name=LN.SCR.browseLoanTypes','id',$ltprofile->id,WEBPAGE::$gt['details'],'recordview',$ltprofile->description),
			WEBPAGE::$gt['tblLoanTypes.payment_frequency'],$ltprofile->payment_frequency_label,
			WEBPAGE::$gt['tblLoanTypes.installment'],$lprofile->installment,
			WEBPAGE::$gt['tblLoanTypes.calendar_type'],$lprofile->calendar_type,
			WEBPAGE::$gt['tblLoanTypes.margin_k'],$lprofile->margin_k,
			WEBPAGE::$gt['tblLoanTypes.margin_r'],$lprofile->margin_r,
			WEBPAGE::$gt['tblLoanTypes.margin_d'],$lprofile->margin_d,
			WEBPAGE::$gt['MSG.OTH.025'],WEBPAGE::$gt['tblLoanTypes.fees_at'],$lprofile->fees_at,
			WEBPAGE::$gt['tblLoanTypes.fees_af'],$lprofile->fees_af,
			WEBPAGE::$gt['MSG.OTH.023'],WEBPAGE::$gt['tblLoanTypes.rates_r'],$lprofile->rates_r,
			WEBPAGE::$gt['tblLoanTypes.rates_d'],$lprofile->rates_d,
			WEBPAGE::$gt['MSG.OTH.024'],$lprofile->rates_e,
			WEBPAGE::$gt['tblCollateral.id'],
			$pg_value_field,
			$re_value_field, 
			$fgd_value_field,
			$fgt_value_field,
			number_format(($lprofile->pg_value + $lprofile->re_value + $lprofile->fgd_value + $lprofile->fgt_value),2,'.',','),
			WEBPAGE::$gt['tblClients.creator_id'],$lprofile->creator,$lprofile->creator_date, 
			WEBPAGE::$gt['tblClients.editor_id'],$lprofile->editor,$lprofile->editor_date,
			$revised_date_field,
			$approval_date_field,
			$disburse_date_field,
...
<?php
	...
	printf ("<TR><TD class=llst>%s</TD><TD class=llst>%s</TD><TD class=llst>%s</TD><TD class=llst>%s</TD></TR>", WEBPAGE::$gt['tblLoanTypes.id'], WEBPAGE::$gt['tblLoanTypes.installment'], WEBPAGE::$gt['tblLoanTypes.payment_frequency'], WEBPAGE::$gt['MSG.OTH.024']);
...
*/	
	...
	switch ( $pq = MAX( 0, count($pmt_plan) - count($nsp_pmt) ))	{
		...
		$data[0][date] = date(WEBPAGE::$gt['MSG.OTH.030'], $truncDateUTC);
		...
		$data[0][date] = date(WEBPAGE::$gt['MSG.OTH.030'], $lastPlan[1]);
		$data[0][prev_balance] = $lastPMT[pk];
		...
		for ($i=0;$i<$pq;$i++) {
			...
			$data[$i][date] = date(WEBPAGE::$gt['MSG.OTH.030'], $pmt_plan[$inx][1]);
...
//			$payment_tmp += $loan->pmt + $delay;						
	...
	$details .= sprintf("<tr style='text-align:center'><td class=lrpt>%s</td><td class=lrpt>%s</td><td class=lrpt>%s</td><td class=lrpt>%s</td><td class=lrpt colspan=2>%s</td><td class=lrpt>%s</td></tr>",WEBPAGE::$gt['tblPayments.date'],WEBPAGE::$gt['MSG.OTH.031'],WEBPAGE::$gt['tblPayments.principal'],WEBPAGE::$gt['tblPayments.interes'],WEBPAGE::$gt['tblPayments.delay'],WEBPAGE::$gt['tblPayments.payment']); 
	$i=0;
	do {
	...
	$buttondata[$c][alt]= WEBPAGE::$gt['addpmt'];
	$buttondata[$c][onClick]="";
	...
	$buttondata[$c][alt]= WEBPAGE::$gt['sheet'];
	$buttondata[$c][onClick]="";
	...
	$buttondata[$c][alt]= WEBPAGE::$gt['MSG.OTH.040'];
	$buttondata[$c][onClick]="";
