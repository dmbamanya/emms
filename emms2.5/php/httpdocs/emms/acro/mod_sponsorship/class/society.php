...
{  	
  ...
  {
  if ($id) { 
    $fields = sprintf("		  			s.*,   			ls.parent_id,		    CONCAT((LPAD(z.id,3,'0')),'.',s.category,'.',(LPAD(s.id,5,'0'))) AS code,		    CONCAT(u.first, ' ', u.middle, ' ', u.last) AS advisor, 		    u.username AS advisor_username,		    CONCAT(uc.first,' ',uc.last) AS creator, 		    DATE_FORMAT(s.creator_date,'%s') AS f_creator_date,		    CONCAT(ue.first,' ',ue.last) AS editor,  		    DATE_FORMAT(s.editor_date,'%s') AS f_editor_date,		    CONCAT(pz.zone,' : ',p.program) AS zone", WEBPAGE::$gt['date_format_mysql'], WEBPAGE::$gt['date_format_mysql']);				    $tables = "tblSocieties AS s, tblUsers AS uc, tblUsers AS ue";              $this->data = current(WEBPAGE::$dbh->getAll(sprintf('    			select %s from (%s)    					left join tblLinkSocieties 	as ls	on s.id 			= ls.child_id AND ls.child_id != ls.parent_id    					left join tblUsers 			as u 	on s.advisor_id 	= u.id    					left join tblZones 			as z 	on z.id 			= s.zone_id   						left join tblZones 			as pz 	on pz.id 			= z.parent_id   						left join tblPrograms 		as p 	on z.program_id 	= p.id     				where s.id = %s AND s.creator_id = uc.id AND s.editor_id = ue.id', $fields, $tables, $id)));    $this->frostdata = array('category');      
    } else {    	
  ...
  if ($soc->data['zone_id']&& $soc->data['advisor_id'] ) {    $c = 0;
    ...
    switch ($soc->data['category']) {  	  case 'BG':        $c++;        $data['buttondata'][$c]['id']	    = "BS.SCR.addSociety";        $data['buttondata'][$c]['href']		= "index.php?scr_name=BS.SCR.addSociety&parent_id=".$id;        $data['buttondata'][$c]['alt']		= WEBPAGE::$gt['newGroup'];        $data['buttondata'][$c]['onClick']	= "";        $data['buttondata'][$c]['ico']		= "society_add";        $c++;        $data['buttondata'][$c]['id']	    = "BS.SCR.deactivateBG";        $data['buttondata'][$c]['href']		= sprintf("index.php?scr_name=BS.SCR.deactivateBG&id=%s&ico=wrn&msg=inf.deactivateBG",$id);        $data['buttondata'][$c]['alt']		= WEBPAGE::$gt['deactivate'];        $data['buttondata'][$c]['onClick']	= "";        $data['buttondata'][$c]['ico']		= "society_rem";        /* Quizas no es buena idea tener esta funcion        $c++;        $data['buttondata'][$c]['id']	    = "BS.SCR.activateBGMember";        $data['buttondata'][$c]['href']		= sprintf('javascript:openWin("index.popup.php?scr_name=BS.SCR.activateBGMember&parent_id=%s","Activate","menubar=no,scrollbars=no,resizable=no,width=400,height=320")',$id);        $data['buttondata'][$c]['alt']		= WEBPAGE::$gt['activate'];        $data['buttondata'][$c]['onClick']	= "";        $data['buttondata'][$c]['ico']		= "activate";        */        break;      case 'B':        $c++;        $data['buttondata'][$c]['id']	    = "BS.SCR.addClient";        $data['buttondata'][$c]['href']		= "index.php?scr_name=BS.SCR.addClient&society_id=".$id;        $data['buttondata'][$c]['alt']		= WEBPAGE::$gt['newClient'];        $data['buttondata'][$c]['onClick']	= "";        $data['buttondata'][$c]['ico']		= "client_add";        $c++;        $data['buttondata'][$c]['id']	    = "BS.SCR.moveSociety";        $data['buttondata'][$c]['href']		= sprintf("index.php?scr_name=BS.SCR.moveSociety&id=%s",$id);        $data['buttondata'][$c]['alt']		= WEBPAGE::$gt['BS.SCR.moveSociety'];        $data['buttondata'][$c]['onClick']	= "";        $data['buttondata'][$c]['ico']		= "society_move";        $c++;        $data['buttondata'][$c]['id']	    = "BS.SCR.deactivateBGMember";        $data['buttondata'][$c]['href']		= sprintf("index.php?scr_name=BS.SCR.deactivateBGMember&parent_id=%s&child_id=%s&ico=wrn&msg=inf.deactivateGroup",$soc->data['parent_id'],$id);        $data['buttondata'][$c]['alt']		= WEBPAGE::$gt['deactivate'];        $data['buttondata'][$c]['onClick']	= "";        $data['buttondata'][$c]['ico']		= "society_rem";       break;      case 'G':        $c++;        $data['buttondata'][$c]['id']	    = "BS.SCR.addClient";        $data['buttondata'][$c]['href']		= "index.php?scr_name=BS.SCR.addClient&society_id=".$id;        $data['buttondata'][$c]['alt']		= WEBPAGE::$gt['newClient'];        $data['buttondata'][$c]['onClick']	= "";        $data['buttondata'][$c]['ico']		= "client_add";        break;        }      } 
  return $data;
  }
  function advisors() 
  {
