...
{ 
  ...
  { 
    if ($id)
    {
      $fields = sprintf("c.*, s.name AS society, s.category,
      ...
      $this->data = current(WEBPAGE::$dbh->getAll(sprintf('
                      select %s
                      from (%s)
                      left join tblSocieties s 	on c.society_id = s.id
                      left join tblUsers     ua on c.advisor_id = ua.id
                      left join tblZones     z 	on z.id         = c.zone_id
                      left join tblZones     pz on pz.id        = z.parent_id
                      left join tblPrograms  p  on z.program_id = p.id
      ...
      {
        $this->data['category'] = $this->data['advisor_id'] ? 'I' : '';
      }
  ...
  {	
    ...
    $params = sprintf("l.client_id      = %s     and
                       l.status        %s '%s'   and
                       l.loan_type_id   = lt.id  and
                       lt.borrower_type = '%s'", $ClientID,$operator,$LoanStatus,$BorrowerType);
