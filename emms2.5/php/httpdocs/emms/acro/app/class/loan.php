...
class LOAN extends WEBPAGE
{
  ...
  if ($id)
  {
    $this->data = current(WEBPAGE::$dbh->getAll(sprintf(
          ...
          where
              ...
              WEBPAGE::$gt['date_format_mysql'], WEBPAGE::$gt['date_format_mysql'],$id)));
  ...
  if ($loan->survey_check()) {
  	$data['survey'] 		= WEBPAGE::$gt['results'];
  	$data['href_survey'] 	= sprintf('index.php?scr_name=SV.SCR.viewGraph&client_id=%s&survey_id=%s',$loan->data['client_id'],WEBPAGE::$conf['app']['survey_id']);
    } else {
 	$data['survey'] 		= WEBPAGE::$gt['applySurvey'];
 	$data['href_survey'] 	= sprintf('index.php?scr_name=SV.SCR.applySurveyStepB&loan_id=%s&client_id=%s&id=%s',$loan->data['id'],$loan->data['client_id'],WEBPAGE::$conf['app']['survey_id']);
   }

  $c=0;
  if (!$loan->isFrost()) {
    $data['buttondata'][$c]['id']	    = "LN.SCR.addILoan";
    $data['buttondata'][$c]['href']		= "index.php?scr_name=LN.SCR.addILoan&id=".$id;
  ...
  $data['buttondata'][$c]['alt']		= WEBPAGE::$gt['LN.SCR.viewLoanMaster'];
  $data['buttondata'][$c]['onClick']	="";
  ...
  if ($loan->data['status']=='G') {
    ...
    $data['buttondata'][$c]['alt']		= WEBPAGE::$gt['LN.SCR.addPayment'];
    $data['buttondata'][$c]['onClick']	="";
  ...
  if ($loan->data['status']=='G') {
    ...
    $data['buttondata'][$c]['alt']		= WEBPAGE::$gt['LN.SCR.addPaymentFull'];
    $data['buttondata'][$c]['onClick']	="";
  ...
  $data['buttondata'][$c]['alt']		= WEBPAGE::$gt['LN.SCR.pmtPlan'];
  $data['buttondata'][$c]['onClick']	="";
  ...
  $data['buttondata'][$c]['alt']		= WEBPAGE::$gt['statusHistory'];
  $data['buttondata'][$c]['onClick']	="";
...
//    $data['buttondata'][$c]['href']		=sprintf('index.php?scr_name=LG.SCR.legalProfile&id=%s',$id);
    $data['buttondata'][$c]['alt']		= WEBPAGE::$gt['LG.SCR.legalProfile'];
    $data['buttondata'][$c]['onClick']	="";
  ...
  function status()
  {
  $opt = explode(",", WEBPAGE::$gt['tblLoans.status.optList']);
  $nam = explode(",", WEBPAGE::$gt['tblLoans.status.optNames']);
  ...
  {
  if ( WEBPAGE::$conf['app']['survey_check'] == '0' ) { return true; }
  ...
  $params = sprintf("survey_id = '%s'
				...
				AND	client_id = '%s'",WEBPAGE::$conf['app']['survey_id'],WEBPAGE::$conf['app']['survey_margin'],$this->data['client_id']);
  return count(SQL::select($tables,$fields,$params));
  }

  function business_check()
  {
    if ( WEBPAGE::$conf['app']['business_check'] == '0' ) { return true; }
  ...
  function load_xp_pmt_plan()
  {
    ...
    $this->data['r_from_date'] = ($this->data['delivered_date'] == '0000-00-00') ? date('Y-m-d') : $this->data['delivered_date'];
    $this->data['xp_pmt_date'] = ($this->data['first_payment_date'] == '0000-00-00') ? $this->next_payment_date($this->data['r_from_date']) : $this->data['first_payment_date'];
  ...
  function getNextPaymentData($mode = '')
  {
    ...
    $data['balance_kp']  = round($this->data['balance_kp']  - $data['principal'],2);
    $data['balance_kaf'] = round($this->data['balance_kaf'] - $data['insurances'],2);
    $data['balance_kat'] = round($this->data['balance_kat'] - $data['fees'],2);
    $this->load_penalties();
    $data['delay']       = $this->data['delay'];
    $data['penalties']   = $this->data['penalties'];
    $data['pmt']         = $this->data['pmt'] + $this->data['penalties'];
    $data['xp_pmt']      = min($this->data['pmt'],$balance + $data['interest']);
  ...
  function load_legal_data()
  {
    ...
    $this->data['legal']['transactions_resume']['legal_credits']        = WEBPAGE::$dbh->getAll(sprintf("select (-1)*sum(total) total, (-1)*sum(principal) principal, (-1)*sum(insurance) insurance, (-1)*sum(fees) fees, (-1)*sum(interest) interest, (-1)*sum(penalties) penalties, (-1)*sum(others) others, '%s' type,'%s' cycle from (
                                                                                                                   ...
                                                                                                                   UNION
                                                                                                                   select IFNULL(sum(lwop.amount),0.00) total, 0.00 principal, 0.00 insurance, 0.00 fees, 0.00 interest, 0.00 penalties, IFNULL(sum(lwop.amount),0.00) others from tblLoanWriteOffPayments lwop where lwop.loan_id = '%s' ) bridge",WEBPAGE::$gt['credits'],WEBPAGE::$gt['legal'],$this->data['id'],$this->data['id']));
    ...
    $this->data['legal']['ini_balances']['regular_ini_balance']          = WEBPAGE::$dbh->getAll(sprintf("select (kp+kaf+kat) total,kp principal,kaf insurance,kat fees,0.00 interest,0.00 penalties,delivered_date date,'%s' cycle from tblLoans  where id = '%s'",WEBPAGE::$gt['regular'],$this->data['id']));
    $this->data['legal']['ini_balances']['parked_ini_balance']           = WEBPAGE::$dbh->getAll(sprintf("select (principal+insurance+fees+interest+penalties) total,principal,insurance,fees,interest,penalties,date,'%s' cycle from tblLoansParked where loan_id = '%s'",WEBPAGE::$gt['legal'],$this->data['id']));
    $this->data['legal']['ini_balances']['write_off_ini_balance']        = WEBPAGE::$dbh->getAll(sprintf("select amount total,principal,insurance,fees,interest,penalties,date,'%s' cycle from tblLoanWriteOff where loan_id = '%s'",WEBPAGE::$gt['writeOff'],$this->data['id']));
    ...
    $this->data['legal']['cause'] = WEBPAGE::$gt[$cause['category']];
